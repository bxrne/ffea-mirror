...
import tkMessageBox
try:
    import tkColorChooser  # Python 2
except:
  ...
  # # # # # # # # # # # # # # # # # # # # # #
    ...
    def load_rod(self, rod, rod_num=0):
        ...
        for i in range(len(rod.current_r)):
            ...
            # TODO: separate each display object of the rod (rod axis, material axis, force axis)
            # draw rod elements
            for j in range(len(rod.current_r[i])):
                ...
                if j < len(rod.current_r[i]) - 1:
                    ...
                    rx2 = rod.current_r[i][j+1][0]
                    ry2 = rod.current_r[i][j+1][1]
                    rz2 = rod.current_r[i][j+1][2]
                    line = line + [CYLINDER, rx, ry, rz, rx2, ry2, rz2, display_radius, color[0], color[1], color[2], color[0], color[1], color[2] ]
                    mid_x = (rx+rx2)/2
                    mid_y = (ry+ry2)/2
                    mid_z = (rz+rz2)/2
                    mvec = np.array([rod.current_m[i][j][0], rod.current_m[i][j][1], rod.current_m[i][j][2]])
                    mhat = mvec / np.linalg.norm(mvec)
                    # material axes in center of each element - light grey
                    if self.display_flags['show_rod_tangent'] == 1:
                        mx = 4 * display_radius * mhat[0]
                        my = 4 * display_radius * mhat[1]
                        mz = 4 * display_radius * mhat[2]
                        line = line + [CYLINDER, mid_x, mid_y, mid_z, mid_x+mx, mid_y+my, mid_z+mz, display_radius/4.0, 0.7, 0.7, 0.7, 0.7, 0.7, 0.7]

                # drawing on nodes
                # steric interaction force - magenta
                if self.display_flags['show_rod_steric_vector'] == 1:
                    # steric force should be same size as material axis
                    svec = np.array([rod.steric_force[i][j][0], rod.steric_force[i][j][1], rod.steric_force[i][j][2]])
                    shat = svec / np.linalg.norm(svec)
                    if abs(np.linalg.norm(svec)) > 0:
                        sx = 4 * display_radius * shat[0]
                        sy = 4 * display_radius * shat[1]
                        sz = 4 * display_radius * shat[2]
                    else:
                        sx = 0
                        sy = 0
                        sz = 0
                    line = line + [CYLINDER, rx, ry, rz, rx+sx, ry+sy, rz+sz, display_radius/4.0, 1, 0, 1, 1, 0, 1]
