...
class FFEA_pdb:
    ...
    def load_pdb(self, fname, num_frames_to_read=100000):
        ...
        self.chain = [FFEA_pdb_chain(num_atoms=num_atoms[i])
        ...
        for c in range(self.num_chains):
            ...
            for j in range(self.chain[c].num_atoms):
                ...
                self.chain[c].atom[j].set_structure(
                    atomID=line[6:12],
                    name=line[12:16],
                    res=line[17:20],
                    resID=line[22:26],
                    ...
                    ffea_comment=self.get_ffea_comment(line))
    ...
    def write_to_file(self, fname, frames=None, frame_rate=1):
        ...
        if ext != ".pdb":
            print_error()
            sys.stdout.write(
                ext + " is not a supported file extension at the moment. Defaulting to .pdb.\n")
        ...
        for i in range(self.num_chains):
            for j in range(self.num_frames):
                try:
                    ...
                    print(
                        "Could not translate, likely due to formatting error in the translation vector provided")
                    raise
   ...
   #        but just frame findex (default 0)
    def rotate_full_system(self, rot, cent=[], findex=0):
        ...
        if findex >= self.num_frames:
            print("Frame " + findex +
                  " does not exist. Please specifiy a correct index")
            raise IndexError

        if cent == []:
            cent = np.array([0.0, 0.0, 0.0])
            for i in range(self.num_chains):
                cent += self.chain[i].frame[findex].calc_centroid() * \
                    self.num_atoms[i]
        ...
        for i in range(self.num_chains):
            for j in range(len(self.chain[i].frame[findex].pos)):
                self.chain[i].frame[findex].pos[j] = np.dot(
                    R, self.chain[i].frame[findex].pos[j])
...
# For FFEA purposes, split object by chains (don't worry about pointers to
# groups of things. If you want that, use MDAnalysis)
...
class FFEA_pdb_atom:
    ...
    def set_structure(self, atomID=0, name="C", res="ARG", resID=0,
                      occupancy=1.0, temperature=0.0, segID="A", element="C",
                      charge="0", ffea_comment=""):
