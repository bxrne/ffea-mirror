...
parser = argparse.ArgumentParser(
    description="Create an FFEA_rod structure and extract its parameters from an all-atom simulation.")
parser.add_argument("prmtop_file", action="store", type=str,
                    help="Input .prmtop (AMBER topology) file.")
parser.add_argument("mdcrd_file", action="store", type=str,
                    help="Input .mdcrd (AMBER trajectory) file.")
parser.add_argument("inpcrd_file", action="store", type=str,
                    help="Input .inpcrd (AMBER co-ordinate) file used for equilibrium configuration.")
parser.add_argument("chain1_end", action="store", type=int,
                    help="the index of the atom at which the first chain (the first coil) in the coiled-coil ends (int).")
parser.add_argument("rod_out", action="store", type=str,
                    help="Path to output file to write (.rodtraj type).")
parser.add_argument("--trj_format", action="store", dest='trj_format', type=str,
                    help="format of the trajectory file (string). Uses MDAnalysis formats.")
parser.add_argument("--inp_format", action="store", dest='inp_format', type=str,
                    help="format of the equilibrium structure file (string). Uses MDAnalysis formats.")
parser.add_argument("--unroll_rod", action="store_true", dest='unroll_rod',
                    help="Whether to unroll the rod (force the equilibrium twist angle to be zero). Note: this will not alter the energies, only the relative values.")  # args.o
parser.add_argument("--get_B", action="store_true", dest='get_B',
                    help="Whether to compute the values of the B matrix (bending stiffness) for each node")  # args.o
parser.add_argument("--get_beta", action="store_true", dest='get_inhomogeneous_beta',
                    help="Whether to compute the values of beta, the twisting stiffness for each node")  # args.o
parser.add_argument("--get_kappa", action="store_true", dest='get_inhomogeneous_kappa',
                    help="Whether to compute the values of kappa, the stretching stiffness for each element")  # args.o
parser.add_argument("--target_length", action="store", dest='target_length', type=int,
                    default="15", help="Target number of nodes in the rod (default 15).")  # args.o
parser.add_argument("--cluster_size", action="store", dest='cluster_size', type=int, default="10",
                    help="Number of atoms which are averaged to create each rod node (default 10).")  # args.o
parser.add_argument("--radius", action="store", dest='radius', type=float,
                    default="5e-9", help="Radius of the rod in m (default 5e-9).")  # args.o
...
def simplify(arr3d, step):
    ...
    if np.shape(averaged_arr3d)[0] == np.shape(arr3d)[0] and np.shape(averaged_arr3d)[2] == np.shape(arr3d)[2] and np.shape(averaged_arr3d)[1] == np.shape(arr3d)[1] / step:
        ...
        raise Exception(
            "Something went wrong. It's Rob Welch's fault. Honestly, this script exists only for his benefit, I don't know why you're using it.")
...
def get_inhomogeneous_param(analysis, parm_index, kbT=300 * 1.38064852e-23):
    ...
    if parm_index == 1:
        ...
        raise IndexError(
            "Supply the index for a parameter (either 1 (twist) or 0 (stretch))")

    #params = 0.5*kbT/(np.average(energies_unit[1:], axis=0))

    params, error = get_avg_and_mean_error(
        energies_unit[1:], kbT * 0.5, per_element=True)
...
def get_delta_omega(analysis, fast=False):
    ...
    def get_mutual_frame_inverse(pim1, pi, weights=None):
        ...
        mutual_element = (1 / weights) * analysis.py_rod_math.normalize(pim1) + \
            (1 / (1 - weights)) * analysis.py_rod_math.normalize(pi)
        return analysis.py_rod_math.normalize(mutual_element)

    def get_mutual_axes_inverse(mim1, mi, pim1=None, pi=None, weights=None):
        if not weights:
            weights = get_weights(pim1, pi)
        m_mutual = (mim1 * (1.0 / weights) + mi * (1.0 / (1 - weights))) / \
            (analysis.py_rod_math.get_length(mi) +
             analysis.py_rod_math.get_length(mim1))
    ...
    for frame in range(len(analysis.p_i)):
        for element in range(len(delta_omega[0])):
            ...
            mim1 = analysis.py_rod_math.normalize(
                analysis.rod.current_m[frame][element])
#            nim1 = np.cross(mim1, pim1/np.linalg.norm(pim1))
            mi = analysis.py_rod_math.normalize(
                analysis.rod.current_m[frame][element + 1])
#            ni = np.cross(mi, pi/np.linalg.norm(pi))
            ...
            equil_mim1 = analysis.py_rod_math.normalize(
                analysis.rod.equil_m[frame][element])
#            equil_nim1 = np.cross(equil_mim1, equil_pim1/np.linalg.norm(equil_pim1))
            equil_mi = analysis.py_rod_math.normalize(
                analysis.rod.equil_m[frame][element + 1])
#            equil_ni = np.cross(equil_mi, equil_pi/np.linalg.norm(equil_pi))

            if fast:
                energy, omega, omega_equil = FFEA_rod.rod_math.get_bend_energy(
                    pim1, pi, equil_pim1, equil_pi, mim1, equil_mim1, mi, equil_mi, np.array([1, 0, 0, 1]), np.array([1, 0, 0, 1]))
                delta_omega[frame][element] = omega - omega_equil
                L_i[frame][element] = (analysis.py_rod_math.get_length(
                    equil_pi) + analysis.py_rod_math.get_length(equil_pim1)) / 2.0
                continue
            ...
            mutual_mi = np.array(analysis.py_rod_math.parallel_transport(
                mi,  analysis.py_rod_math.normalize(pi), mutual_l))[0]
            mutual_mim1 = np.array(analysis.py_rod_math.parallel_transport(
                mim1,  analysis.py_rod_math.normalize(pim1), mutual_l))[0]

            # get the weighted average to create the mutual material axes
            mutual_m_rotated = get_mutual_axes_inverse(
                mutual_mim1, mutual_mi, weights=weight)

            omega, kb = analysis.py_rod_math.omega(pi, pim1, np.cross(
                mutual_l, mutual_m_rotated), mutual_m_rotated)
            ...
            equil_mutual_l = get_mutual_frame_inverse(
                equil_pim1, equil_pi, weights=equil_weight)

            # parallel transport our two existing material axes onto our new element
            equil_mutual_mi = np.array(analysis.py_rod_math.parallel_transport(
                equil_mi, analysis.py_rod_math.normalize(equil_pi), equil_mutual_l))[0]
            equil_mutual_mim1 = np.array(analysis.py_rod_math.parallel_transport(
                equil_mim1, analysis.py_rod_math.normalize(equil_pim1), equil_mutual_l))[0]

            # get the weighted average to create the mutual material axes
            equil_mutual_m_rotated = get_mutual_axes_inverse(
                equil_mutual_mim1, equil_mutual_mi, weights=equil_weight)

            equil_omega, equil_kb = analysis.py_rod_math.omega(equil_pi, equil_pim1, np.cross(
                equil_mutual_l, equil_mutual_m_rotated), equil_mutual_m_rotated)

            delta_omega[frame][element] = np.array(
                omega - equil_omega).transpose()[0]
...
def get_avg_and_mean_error(rod_energy, half_kBT, per_element=False):
    ...
    if per_element:
        ...
        standard_error = (half_kBT / np.std(rod_energy, axis=0)
                          ) / np.sqrt(len(rod_energy))
    else:
        quantity = half_kBT / np.nanmean(rod_energy)
        standard_error = (half_kBT / np.std(rod_energy)) / \
            np.sqrt(len(rod_energy))
...
def get_B_avg(delta_omega, temperature, li, element_no, covariance=True, get_C=False):
    ...
    if covariance:
        C11 = np.average(delta_omega_element[:, 0] * delta_omega_element[:, 0]) - (
            np.average(delta_omega_element[:, 0]) * np.average(delta_omega_element[:, 0]))
        C12_21 = np.average(delta_omega_element[:, 0] * delta_omega_element[:, 1]) - (
            np.average(delta_omega_element[:, 0]) * np.average(delta_omega_element[:, 1]))
        C22 = np.average(delta_omega_element[:, 1] * delta_omega_element[:, 1]) - (
            np.average(delta_omega_element[:, 1]) * np.average(delta_omega_element[:, 1]))
    else:
        C11 = np.average(delta_omega_element[:, 0] * delta_omega_element[:, 0])
        C12_21 = np.average(
            delta_omega_element[:, 0] * delta_omega_element[:, 1])
        C22 = np.average(delta_omega_element[:, 1] * delta_omega_element[:, 1])
    C = np.matrix([[C11, C12_21], [C12_21, C22]])
    try:
        B = (1.38064852e-23 * temperature *
             (np.average(li_element))) * np.linalg.inv(C)
...
def iterative_improve_B(original_rod, new_rod, write_out_file="", extra_data=True, alpha=1.0):
    ...
    for element_no in range(len(old_delta_omega[0])):
        curr_old_B, curr_old_C = get_B_avg(
            old_delta_omega, temp, old_L_i, element_no, get_C=True, covariance=True)
    ...
    for element_no in range(len(new_delta_omega[0])):
        curr_new_B, curr_new_C = get_B_avg(
            new_delta_omega, temp, new_L_i, element_no, get_C=True, covariance=True)
    ...
    for i in range(len(new_B)):
        corrected_B.append(analytical_kbT * np.average(new_L_i, 0)[i] * np.linalg.inv(alpha * (
            old_C[i] - new_C[i]) + (analytical_kbT * np.average(new_L_i, 0)[i] * np.linalg.inv(old_B[i]))))
    ...
    if write_out_file != "":
        ...
        fixed_rod.B_matrix[0][1:-1] = np.reshape(np.array(corrected_B).flatten(), [
                                                 len(np.array(corrected_B).flatten()) / 4, 4])
...
def recover_B(rodtraj_path, new_rod_path=None, fast=True):
    ...
    if new_rod_path:
        ...
        rod.B_matrix[0][1:-1] = np.reshape(np.array(B).flatten(),
                                           [len(np.array(B).flatten()) / 4, 4])
...
def complete_parallel_transport(analysis, from_index, to_index, equil=True, force_normalize=True):
    ...
    for i in range(from_index, to_index - 1):
        curr_mataxis = np.asarray(FFEA_rod.rod_math.parallel_transport(
            curr_mataxis, n(p[0][i]), n(p[0][i + 1])))[0]
    return curr_mataxis
...
def unroll(analysis, perpendicularize=True, do_complete_parallel_transport=False):
    ...
    for frame in range(len(rod.current_m)):
        for elem in range(len(rod.current_m[frame]) - 1):
            rod.current_m[frame][elem] = FFEA_rod.rod_math.normalize(
                rod.current_m[frame][elem])
            rod.equil_m[frame][elem] = FFEA_rod.rod_math.normalize(
                rod.equil_m[frame][elem])

    print("Perpendicularizing...")
    if perpendicularize:
        for frame in range(len(rod.current_m)):
            for elem in range(len(rod.current_m[frame]) - 1):
                rod.current_m[frame][elem] = FFEA_rod.rod_math.perpendicularize(
                    rod.current_m[frame][elem], analysis.p_i[frame][elem])
                rod.equil_m[frame][elem] = FFEA_rod.rod_math.perpendicularize(
                    rod.equil_m[frame][elem], analysis.p_i_equil[frame][elem])
    ...
    for mataxis_index in range(len(rod.equil_m[0]) - 1):
        if do_complete_parallel_transport:
            transported_mataxis = complete_parallel_transport(
                analysis, 0, mataxis_index, equil=True, force_normalize=True)
            curr_mataxis = FFEA_rod.rod_math.normalize(
                rod.equil_m[0][mataxis_index])
            curr_elem = FFEA_rod.rod_math.normalize(
                analysis.p_i_equil[0][mataxis_index])
            rotation_table[mataxis_index] = FFEA_rod.rod_math.get_signed_angle(
                transported_mataxis, curr_mataxis, curr_elem)

        else:
            transported_mataxis = np.squeeze(np.asarray(FFEA_rod.rod_math.parallel_transport(FFEA_rod.rod_math.normalize(
                rod.equil_m[0][mataxis_index]), FFEA_rod.rod_math.normalize(analysis.p_i_equil[0][mataxis_index]), elem1)))
            # note: transported mataxis is np array type
            rotation_table[mataxis_index] = FFEA_rod.rod_math.get_signed_angle(
                mat1, transported_mataxis, elem1)
    ...
    for frame in range(len(rod.current_m)):
        ...
        for elem in range(len(rod.current_m[frame]) - 1):
            ...
            rod.current_m[frame][elem] = FFEA_rod.rod_math.rodrigues(
                FFEA_rod.rod_math.normalize(rod.current_m[frame][elem]), k, rotation_table[elem])
            k_equil = FFEA_rod.rod_math.normalize(
                analysis.p_i_equil[frame][elem])
            rod.equil_m[frame][elem] = FFEA_rod.rod_math.rodrigues(
                FFEA_rod.rod_math.normalize(rod.equil_m[frame][elem]), k_equil, rotation_table[elem])
...
def main(prmtop_file, mdcrd_traj_file, inpcrd_file, chain1_end, trj_format="TRJ", inp_format="INPCRD", get_B=True, target_length=15, cluster_size=10, simplify_only=False, rod=None, radius=5e-9, rod_out=None, unroll_rod=True, get_inhomogenous_beta=True, get_inhomogenous_kappa=True):
    ...
    if not rod:
        ...
        u_initial = MDAnalysis.Universe(
            topology, inpcrd_file, format=inp_format)
        initial_backbone = u_initial.select_atoms(
            'protein and backbone').positions / 1e10
...
#        chain2_hinge_len = len(chain2_hinge)
        ...
        def set_cluster_indices(target_length, cluster_size, chain1_end):
            ...
            cluster_indices = np.zeros(
                [target_length, cluster_size], dtype=int)
            starting_cluster_index = int(cluster_size / 2)
            end_cluster_index = chain1_end - int(cluster_size / 2)
            cluster_center_indices = np.linspace(
                starting_cluster_index, end_cluster_index, target_length, dtype=int)
        ...
        def get_node_traj(cluster_indices, node_index, backbone_traj, nearest_node_table, chain1_end):
            ...
            for curr_node_index in cluster_indices[node_index]:
                ...
                chain2_nodes[i] = backbone_traj[:,
                                                nearest_node_table[curr_node_index - 1]]
                i += 1
            chain1_avg = np.average(chain1_nodes, axis=0)
            chain2_avg = np.average(chain2_nodes, axis=0)

            # setup mataxis_cluster
            mataxis_chain1_nodes = np.zeros(
                [cluster_size, len(backbone_traj), 3])
            mataxis_chain2_nodes = np.zeros(
                [cluster_size, len(backbone_traj), 3])
            mataxis_offset = int(np.average(
                cluster_indices[1] - cluster_indices[0]) / 2.0)
            ...
            for curr_node_index in cluster_indices[node_index]:
                ...
                mataxis_chain1_nodes[j] = backbone_traj[:,
                                                        curr_node_index + mataxis_offset]
                mataxis_chain2_nodes[j] = backbone_traj[:,
                                                        nearest_node_table[curr_node_index + mataxis_offset - 1]]
                j += 1
        ...
        cluster_indices = set_cluster_indices(
            target_length, cluster_size, chain1_end)
        print("Calculating nearest node table...")
        nearest_node_table = get_nearest_node_table(
            initial_backbone, chain1_end)
        ...
        for node_index in range(target_length):
            rod_r[:, node_index], rod_m[:, node_index] = get_node_traj(
                cluster_indices, node_index, backbone_traj, nearest_node_table, chain1_end)

        equil_r = np.zeros([len(backbone_traj), target_length, 3])
        equil_m = np.zeros([len(backbone_traj), target_length, 3])
        for node_index in range(target_length):
            equil_r[:, node_index], equil_m[:, node_index] = get_node_traj(
                cluster_indices, node_index, np.array([initial_backbone]), nearest_node_table, chain1_end)
        ...
        rod.perturbed_x_energy_positive = np.zeros(
            [len(rod.current_m), len(rod.current_m[0]), 3])
        rod.perturbed_y_energy_positive = np.zeros(
            [len(rod.current_m), len(rod.current_m[0]), 3])
        rod.perturbed_z_energy_positive = np.zeros(
            [len(rod.current_m), len(rod.current_m[0]), 3])
        rod.perturbed_x_energy_negative = np.zeros(
            [len(rod.current_m), len(rod.current_m[0]), 3])
        rod.perturbed_y_energy_negative = np.zeros(
            [len(rod.current_m), len(rod.current_m[0]), 3])
        rod.perturbed_z_energy_negative = np.zeros(
            [len(rod.current_m), len(rod.current_m[0]), 3])
        rod.twisted_energy_positive = np.zeros(
            [len(rod.current_m), len(rod.current_m[0]), 3])
        rod.twisted_energy_negative = np.zeros(
            [len(rod.current_m), len(rod.current_m[0]), 3])
    ...
    avg_stretch, stretch_error = get_avg_and_mean_error(
        analysis.stretch_energy, half_kbT)
    avg_bend, bend_error = get_avg_and_mean_error(
        analysis.bending_energy, half_kbT)
    avg_twist, twist_error = get_avg_and_mean_error(
        analysis.twist_energy, half_kbT)

    print("Done.")
    print("Kappa (stretch constant)    = " +
          str(kappa_stretch) + "+/-" + str(stretch_error))
    print("B (isotropic bend constant) = " +
          str(B_isotropic) + "+/-" + str(bend_error))
    print("beta (twist constant)       = " +
          str(beta) + "+/-" + str(twist_error))
    ...
    if get_inhomogenous_beta:
        inhomogeneous_beta, beta_err = get_inhomogeneous_param(
            analysis, 1, kbT=analytical_kbT)
        inhomogeneous_beta = np.concatenate(
            [[inhomogeneous_beta[0]], inhomogeneous_beta, [inhomogeneous_beta[-1]]])
    ...
    if get_inhomogenous_kappa:
        inhomogeneous_kappa, kappa_err = get_inhomogeneous_param(
            analysis, 0, kbT=analytical_kbT)
        inhomogeneous_kappa = np.concatenate(
            [inhomogeneous_kappa, [inhomogeneous_kappa[-1]]])
...
if __name__ == '__main__' and '__file__' in globals():
    ...
    out = main(args.prmtop_file, args.mdcrd_file, args.inpcrd_file, args.chain1_end, args.trj_format, args.inp_format, get_B=args.get_B, target_length=args.target_length, cluster_size=args.cluster_size,
               simplify_only=False, rod=None, radius=args.radius, rod_out=args.rod_out, unroll_rod=args.unroll_rod, get_inhomogenous_beta=args.get_inhomogeneous_beta, get_inhomogenous_kappa=args.get_inhomogeneous_kappa)
    print(out)
